...
/**
 * Created by tyxo on 2016/7/21 11: 14.
...
public class ViewUtil {
    ...
    public static void getImageFromAlbum(Activity context) {
        ...
        context.startActivityForResult(intent, CODE_REQUEST_IMAGE);
    }
    ...
    public static void getImageFromCamera(Activity activity) {
        ...
        if (state.equals(Environment.MEDIA_MOUNTED)) {
            ...
            activity.startActivityForResult(intent, CODE_REQUEST_CAMERA);
        } else {
    ...
    public static void getImageFromCameraBig(Activity activity) {
        ...
        if (state.equals(Environment.MEDIA_MOUNTED)) {
            Intent intent = new Intent("android.media.action.IMAGE_CAPTURE");
            String out_file_path = SAVE_IMAGE_DIR_PATH;
            File dir = new File(out_file_path);
            if (!dir.exists()) {
                dir.mkdirs();
            }
            SAVE_IMAGE_DIR_PATH_TEMP = SAVE_IMAGE_DIR_PATH + System.currentTimeMillis() + ".jpg";
            intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(getImageFile(SAVE_IMAGE_DIR_PATH_TEMP)));
            intent.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 1);//加上这两句,onActivityResult里的data会报空
            activity.startActivityForResult(intent, CODE_REQUEST_CAMERABIG);
        } else {
            ToastUtil.showToastS(activity, "请确认已经插入SD卡");
        }
    }
     ...
     */
    public static void initBottomRightMenu(final Activity activity, final ScrollView scrollView, int id) {
        ...
        /*fabIconNew.setImageDrawable(activity.getResources().getDrawable(id));
    ...
    public static void initCircleMenu(Activity activity) {
        ...
        TextView f = new TextView(activity);
        f.setText("f");
        f.setBackgroundResource(android.R.drawable.btn_default_small);
        TextView g = new TextView(activity);
        g.setText("g");
