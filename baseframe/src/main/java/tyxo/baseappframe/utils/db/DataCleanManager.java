...
public class DataCleanManager implements IDialogAction {
    ...
    public static void cleanInternalCache(Context context) {
        Log.i(TAG, "缓存路径：" + context.getCacheDir());
        /* /data/data/com.haihong.haihongtradeapp/cache */
        Log.i(TAG, "删除webview缓存文件..."); // Android 4.4.2中
        ...
        if (Build.VERSION.SDK_INT >= 18) {
            ...
            Log.i(TAG, "4.4+webview缓存文件路径..." + appWebViewPath); // Android
            // 4.4.2中
            // WebView的缓存文件不在cache文件夹下
    ...
    public static void cleanApplicationData(final Context context,
        ...
        final Handler handler = new Handler() {
            public void handleMessage(Message msg) {
                if (msg.what == 1) {
                    Toast.makeText(context, "数据清除成功", 0).show();
                } else {
                    Toast.makeText(context, "数据清除失败", 0).show();
                }
            }
        };
        new Thread() {
...
//					ImageLoader.getInstance().clearMemoryCache();
                    // 清除网络访问数据缓存
                    /**因打jar包工具类,注释掉了*///VolleyManager.getInstance(context).clear();
    ...
    public static void cleanUserData(Context context, String... filepaths) {
        ...
        // 清除Imageloader 本地缓存
        /**因打jar包工具类,注释掉了*///ImageLoader.getInstance().clearDiskCache();
        // 清除Imageloader 内存缓存
        /**因打jar包工具类,注释掉了*///ImageLoader.getInstance().clearMemoryCache();
        // 清除网络访问数据缓存
        /**因打jar包工具类,注释掉了*///VolleyManager.getInstance(context).clear();
        ...
        if (dir != null && dir.isDirectory()) {
            try {
                for (File child : dir.listFiles()) {
                    ...
                    Log.i(TAG, "删除缓存文件..." + child.getName());
                }
            } catch (Exception e) {
