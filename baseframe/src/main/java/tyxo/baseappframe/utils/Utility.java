...
public class Utility {
    ...
    public static String switchOrderDetailList(Context context, String ordertype, TextView tv, int colorId) {
        switch (ordertype) {
            case "":
                //tv.setTextColor(context.getResources().getColor(R.color.xxx));
    ...
    public static String switchOrderDetailDegree(Context context, String ordertype, TextView tv, int colorId) {
        switch (ordertype) {
            case "":
                return "";
            case "10":
    ...
    /**
     * @param colorId 首位是紧急,后面是正常灰色.
     * @author ly
     * @created at 2016/9/19 17:13
     * @des : 根据传入的状态,设置字体颜色
     */
    public static void setTextColor(Context context, String degree, TextView tv, int... colorId) {
        if (colorId.length < 2) {
            return;
        }
        if ("紧急".equals(degree)) {
            //tv.setTextColor(context.getResources().getColor(R.color.xxx));
            //tv.setTextColor(0xea5967);
            tv.setTextColor(colorId[0]);
        } else {
            //tv.setTextColor(context.getResources().getColor(R.color.xxx));
            //tv.setTextColor(0x333333);
            tv.setTextColor(colorId[1]);
        }
    }
    ...
    // 发货详情界面,删除+修改 按钮显示状态
    public static void switchDeliveryDeleteVisible(Context context, TextView delete, TextView modify, String state, int... ids) {
        if (ids.length < 4) {
            return;
        }
    ...
    public static void setTextViewState(Context context, TextView view, boolean enable, int... ids) {
