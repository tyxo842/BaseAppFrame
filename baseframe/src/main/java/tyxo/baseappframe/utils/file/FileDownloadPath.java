package tyxo.baseappframe.utils.file;
...
/**
 * @ClassName:FileDownloadPath 文件下载根目录的管理，包含对SD卡插拔情况的处理
...
public class FileDownloadPath {
    ...
    public static class SubPath {
        ...
        public String getPath(Context context, boolean forceInternal) {
            if (forceInternal) {

                // 取下载路径
                mSubPathInternal = getRootPath(context, forceInternal) + subDirName;
                ...
                // 取下载路径
                mSubPath = getRootPath(context, forceInternal) + subDirName;
                File f = new File(mSubPath);
                if (!f.exists()) {
                    boolean ret = f.mkdirs();
                    Log.d(TAG, "create dir" + mSubPath + ret);
                }
        ...
        private void reGetPath(Context context) {
            mSubPath = null;
            mSubPath = getPath(context, false);
            mSubPathInternal = null;
            mSubPathInternal = getPath(context, true);
        }
    }
     ...
     * @return
     */
    private static String getRootPath(Context context, boolean forceInternal) {
        if (forceInternal) {
            if (mRootPathInternal == null || mRootPathInternal.length() == 0) {
                mRootPathInternal = getStorePath(context, ROOT_NAME, forceInternal);
            }
            return mRootPathInternal;
        } else {
            if (mRootPath == null || mRootPath.length() == 0) {
                mRootPath = getStorePath(context, ROOT_NAME, forceInternal);
            }
            return mRootPath;
        }
    }
