package tyxo.baseappframe.base;
...
import tyxo.baseappframe.net.TaskHelp;
import tyxo.baseappframe.net.volley.VolleyCallBack;
import tyxo.baseappframe.net.volley.VolleyErrorResult;
import tyxo.baseappframe.utils.ToastUtil;
 ...
 *            参考 GirlsActivity 代码.
 */
public abstract class BaseRecyclerActivity<T extends Object> extends BaseActivityToolbar implements
    ...
    protected void requestNet(){
        ...
        callback =  new VolleyCallBack<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                TLog.i("tyxo", " response : " + response.toString());
                try {
                    if (!TextUtils.isEmpty(response.toString())) {
                        ...
                        TLog.i("tyxo", " response为空 返回信息: " + "服务器返回结果为空");
                    }
                } catch (Exception e) {
                    stopLoading();
                    e.printStackTrace();
                    TLog.i("tyxo", " 解析数据错误" + e.getMessage());
                }
            }
            ...
            public void onErrorResponse(VolleyErrorResult result) {
                stopLoading();
                TLog.i("tyxo", " 请求失败: " + result.toString());
            }
        };
    ...
    protected T parseObject(String response) throws Exception {
        try {
            ...
            TLog.e("tyxo","BaseRecyclerActivity parseObject 数据解析失败"+e);
